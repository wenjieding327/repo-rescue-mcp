...
清单以对应提交中 Git blob 的原始字节为准（当前文本文件为 UTF-8、LF），二进制文件不得转码。生成源码包时使用 `git -c core.autocrlf=false archive --format=zip --output=source.zip <commit>`，再对导出文件逐项验证；Windows 的 `core.autocrlf=true` 也会影响 `git archive`，因此仅写“从固定提交导出”不足以保证字节一致。不要以 CRLF 工作区副本重算清单来掩盖导出转换，也不要修改历史标签、历史运行证据或原始工作流内容。
